...
TEST(BuildInfoTests, CurrentBuildInfoContainsProtocolAndStorageVersions) {
  ...
  EXPECT_EQ(info.protocol_version, "4");
  EXPECT_EQ(info.storage_format_version, "2");
}
...
TEST(BuildInfoTests, JsonContainsExpectedKeys) {
  ...
  EXPECT_NE(json.find("\"protocol_version\":\"4\""), std::string::npos);
  EXPECT_NE(json.find("\"storage_format_version\":\"2\""), std::string::npos);
