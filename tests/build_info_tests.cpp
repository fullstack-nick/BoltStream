...
TEST(BuildInfoTests, CurrentBuildInfoContainsPhaseOneVersions) {
  const auto info = boltstream::current_build_info();

  EXPECT_EQ(info.service, "boltstream");
  EXPECT_FALSE(info.version.empty());
  EXPECT_FALSE(info.git_sha.empty());
  EXPECT_EQ(info.protocol_version, "0");
  EXPECT_EQ(info.storage_format_version, "0");
}
...
TEST(BuildInfoTests, JsonContainsExpectedKeys) {
  ...
  EXPECT_NE(json.find("\"protocol_version\":\"0\""), std::string::npos);
  EXPECT_NE(json.find("\"storage_format_version\":\"0\""), std::string::npos);
