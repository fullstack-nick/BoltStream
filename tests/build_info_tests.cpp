...
TEST(BuildInfoTests, CurrentBuildInfoContainsProtocolAndStorageVersions) {
  ...
  EXPECT_EQ(info.storage_format_version, "3");
}
...
TEST(BuildInfoTests, JsonContainsExpectedKeys) {
  ...
  EXPECT_NE(json.find("\"protocol_version\":\"5\""), std::string::npos);
  EXPECT_NE(json.find("\"storage_format_version\":\"3\""), std::string::npos);
